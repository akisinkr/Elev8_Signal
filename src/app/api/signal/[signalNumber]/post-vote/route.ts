...
function buildSuperpowerLine(member: { spDomain?: string | null; spAction?: string | null }): string {
  const parts = [member.spDomain, member.spAction].filter(Boolean);
  return parts.length > 0 ? parts.join(" · ") : "Cross-functional Leadership";
}

function buildCanHelpWith(member: { spDomain?: string | null; spAction?: string | null }): string {
  if (member.spAction) return member.spAction;
  if (member.spDomain) return member.spDomain;
