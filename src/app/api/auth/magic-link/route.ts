...
export async function POST(req: NextRequest) {
  try {
    ...
    // Dev mode: log OTP to console so you can test without email
    if (process.env.NODE_ENV === "development") {
      console.log(`\n🔑 [DEV] OTP for ${normalizedEmail}: ${code}\n`);
    }

    // Send email (don't block on failure in dev mode)
    try {
      await sendEmail({
        to: normalizedEmail,
      ...
      // In dev mode, continue anyway since OTP is logged to console
